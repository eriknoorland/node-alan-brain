  ...
  return ({ motors, encoders }) => {
    ...
    function rotate(angle, direction = 'left') {
      return new Promise((resolve) => {
        ...
        const numTargetRotationTicks = Math.round(config.encoders.numFullRotationTicks / (360 / angle));
        const goal = config.speed.rotate;
        ...
        feedbackLoopInterval = setInterval(() => {
          ...
          if (encoderCountLeft >= numTargetRotationTicks
            || encoderCountRight >= numTargetRotationTicks) {
            resolve();
          }
        }, loopTime);
      });
    ...
    function straight(direction = 'forward') {
      ...
      const goal = config.speed.straight;
